...
def combine_vars(v1, v2, mu1, mu2, m, n):
    ...
    return (
        (m / (m + n)) * v1 + n / (m + n) * v2 + m * n / (m + n) ** 2 * (mu1 - mu2) ** 2
    )
...
def extract_stat_data(files_list, config, sample_rate, num_samples):
    ...
    for i in tqdm(range(len(files_list))):
        ...
        if i == 0:
            ...
            mean_new = combine_means(
                mean_curr, torch.mean(mel_spec), N_curr, N_new
            )  # Mean of previous spectrograms + new
            var_new = combine_vars(
                var_curr,
                torch.var(mel_spec),
                mean_curr,
                torch.mean(mel_spec),
                N_curr,
                N_new,
            )  # Var of previous spectrograms + new
